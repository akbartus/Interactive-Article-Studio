...
constructor() {
  ...
  // Add event listeners (now safe - checks for existence)
  ...
  // Legacy method - alignment icons removed in new Annotations panel
  return;
}
  addColorPickerListener() {
  // Legacy method - color picker removed in new Annotations panel
  return;
}
  
  addTooltipEventListeners() {
  // These inputs no longer exist in the new Annotations panel
  // Keep method for backward compatibility but make it safe
  return;
}

updateSelectedTooltip() {
  // Legacy method - no longer used with new Annotations system
  return;
}
...
select(entity, add = false) {
  ...
  if (entity && !this.selectedEntities.includes(entity)) {
...
updateSelectionUI() {
  ...
  // Annotations now managed separately
  return;
}
  ...
  deleteSelectedEntities() {
    if (this.selectedEntities.length > 0) {
      ...
      this.clearSelection();
      console.log("Selected entities deleted.");
    } else {
      console.log("No entities selected to delete.");
    }
  }
  createCamera() {
  ...
  addEntity(entity) {
    ...
    this.entities.push(entity.object3D);
    return entity;
  }
  rodationYChange(y) {
    if (this.selectedEntities.length === 1) {
      this.selectedEntities[0].rotation.y = THREE.Math.degToRad(y);
    }
  }
  rodationXChange(x) {
    if (this.selectedEntities.length === 1) {
      this.selectedEntities[0].rotation.x = THREE.Math.degToRad(x);
      this.viewport.selectionBoxes[0].setFromObject(this.selectedEntities[0]).update();
    }
  }
  widthChange(size) {
    if (this.selectedEntities.length === 1) {
      this.selectedEntities[0].scale.set(
        size,
        this.selectedEntities[0].scale.y,
        this.selectedEntities[0].scale.z
      );
      this.viewport.selectionBoxes[0].setFromObject(this.selectedEntities[0]).update();
    }
  }
  heightChange(size) {
    if (this.selectedEntities.length === 1) {
      this.selectedEntities[0].scale.set(
        this.selectedEntities[0].scale.x,
        size,
        this.selectedEntities[0].scale.z
      );
      this.viewport.selectionBoxes[0].setFromObject(this.selectedEntities[0]).update();
...
class Viewport {
  constructor(editor) {
    ...
    this.selectionBoxes = [];
    this.pivot = null;
    this.initialPivotPos = null;
    this.initialPivotQuat = null;
    this.initialPivotScale = null;
    ...
    const handleLeftClick = (event) => {
      if (this.onDownPosition.distanceTo(this.onUpPosition) === 0) {
        ...
        if (intersects.length > 0) {
          ...
          if (object && object.el) {
            if (event.shiftKey) {
              if (this.editor.selectedEntities.includes(object)) {
                this.editor.selectedEntities = this.editor.selectedEntities.filter(
                  (ent) => ent !== object
                );
              } else {
                this.editor.select(object, true);
              }
            } else {
  ...
  updateTransform() {
    ...
    const selected = this.editor.selectedEntities;
    ...
    if (selected.length === 1) {
      ...
      if (entity.el && entity.el.tagName !== "A-SKY") {
        const box = new THREE.BoxHelper(entity, 0x1faaf2);
        box.material.depthTest = false;
        box.material.transparent = true;
        this.editor.scene.add(box);
        this.selectionBoxes.push(box);
        box.setFromObject(entity).update();
      ...
      selected.forEach((ent) => {
        avgPos.add(ent.position);
      });
      avgPos.divideScalar(selected.length);
      this.pivot.position.copy(avgPos);
      this.pivot.quaternion.copy(new THREE.Quaternion());
      this.pivot.scale.set(1, 1, 1);
      this.transformControls.attach(this.pivot);
      // Create selection boxes for each
      selected.forEach((ent) => {
        if (ent.el && ent.el.tagName !== "A-SKY") {
          const box = new THREE.BoxHelper(ent, 0x1faaf2);
          box.material.depthTest = false;
          box.material.transparent = true;
          this.editor.scene.add(box);
          this.selectionBoxes.push(box);
          box.setFromObject(ent).update();
        }
      });
    }
  }
  ...
  createTransformControls() {
    ...
    this.transformControls.addEventListener("objectChange", (evt) => {
      ...
      if (object === this.pivot) {
        ...
        const deltaPosition = this.pivot.position.clone().sub(this.initialPivotPos);
        const deltaQuaternion = this.pivot.quaternion.clone().multiply(this.initialPivotQuat.clone().invert());
        const deltaScale = this.pivot.scale.clone().divide(this.initialPivotScale);
        for (let i = 0; i < selected.length; i++) {
          const ent = selected[i];
          const init = this.initialStates[i];
          const relPos = init.position.clone().sub(this.initialPivotPos);
          relPos.applyQuaternion(deltaQuaternion);
          relPos.multiply(deltaScale);
          ent.position.copy(this.initialPivotPos).add(deltaPosition).add(relPos);
          ent.quaternion.copy(deltaQuaternion).multiply(init.quaternion).normalize();
          ent.scale.copy(init.scale).multiply(deltaScale);
          if (ent.el.id.startsWith("splat")) {
            ...
            document.dispatchEvent(
              new CustomEvent("objectRotationUpdate", {
                detail: {
                  id: ent.el.id,
                  x: (ent.rotation.x * 180) / Math.PI,
                  y: (ent.rotation.y * 180) / Math.PI,
                  z: (ent.rotation.z * 180) / Math.PI,
                },
              })
            );
          }
          this.selectionBoxes[i].setFromObject(ent).update();
        }
      } else {
        // Single object
        ...
        this.selectionBoxes[0].setFromObject(object).update();
      }
    });
    this.transformControls.addEventListener("mouseDown", () => {
      this.editorControls.enabled = false;
      const selected = this.editor.selectedEntities;
      if (selected.length > 1 && this.pivot) {
        this.initialPivotPos = this.pivot.position.clone();
        this.initialPivotQuat = this.pivot.quaternion.clone();
        this.initialPivotScale = this.pivot.scale.clone();
        this.initialStates = selected.map((ent) => ({
          position: ent.position.clone(),
          quaternion: ent.quaternion.clone(),
          scale: ent.scale.clone(),
        }));
      }
    });
